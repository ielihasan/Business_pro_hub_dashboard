-- ============================================
-- ROLE-BASED AUTHENTICATION UPDATE (FIXED)
...
DROP POLICY IF EXISTS "Business owners can manage own queues" ON public.queues;
DROP POLICY IF EXISTS "Customers can view own queue entries" ON public.queues;
...
-- Customers can view their own queue entries (if they have customer_id)
...
-- Drop existing policies
DROP POLICY IF EXISTS "Business owners can manage own feedback" ON public.feedback;
DROP POLICY IF EXISTS "Customers can manage own feedback" ON public.feedback;
...
-- Drop existing policies
DROP POLICY IF EXISTS "Business owners can manage own hours" ON public.business_hours;
...
DROP POLICY IF EXISTS "Admins can view all hours" ON public.business_hours;
...
SELECT 'Ready to build Smart Business Support Platform!' as next_step;
