...
- **Java 17 (JDK)** — https://adoptium.net (Eclipse Temurin recommended)
- **Apache Maven 3.9+** — https://maven.apache.org/download.cgi
- **Supabase account** (free tier) — https://supabase.com
- **Resend account** (free tier, for email) — https://resend.com
...
3. **Configure frontend environment variables:**
   ...
   # Then edit dashboard/.env.local with your values
   ```
...
6. **Start the Spring Boot backend** (terminal 1):
   ```bash
   cd backend
   # Load env vars and start (Linux/macOS/Git Bash)
   export $(grep -v '^#' .env | xargs) && mvn spring-boot:run

   # Windows CMD
   # Set each variable manually then: mvn spring-boot:run
   ```
...
8. **Access the application:**
   ...
   - **Login**: http://localhost:3002/auth/v1/login
   - **Register**: http://localhost:3002/auth/v1/register
   - **Admin Dashboard**: http://localhost:3002/admin/dashboard
   - **Business Dashboard**: http://localhost:3002/business/dashboard
...
│   │   │   └── join-queue/      # Public queue join page (QR scan)
...
│   ├── src/main/java/com/businessprohub/backend/
│   │   ├── controller/          # REST controllers (Queue, Auth, Pricing, …)
│   │   ├── service/             # Business logic
│   │   ├── entity/              # JPA entities (Queue, Business, …)
...
# Backend dependencies are handled by Maven automatically on first run
```
...
# Windows CMD (set vars manually or use a .bat wrapper)
mvn spring-boot:run
```
...
| `NEXT_PUBLIC_APP_URL` | Frontend URL | `http://localhost:3002` for development |
| `NEXT_PUBLIC_API_URL` | Spring Boot backend URL | `http://localhost:8080` for development |
| `RESEND_API_KEY` | Resend email API key | [resend.com/api-keys](https://resend.com/api-keys) |
| `RESEND_FROM_EMAIL` | Email sender address | `onboarding@resend.dev` for dev |
| `ADMIN_NOTIFICATION_EMAIL` | Email for admin alerts | Any valid email |

### Backend (`backend/.env`)

| Variable | Description | Where to Get |
|----------|-------------|--------------|
| `SUPABASE_DB_PASSWORD` | Supabase Postgres password | Supabase Dashboard → Settings → Database |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase service role key | Supabase Dashboard → Settings → API |
| `SUPABASE_JWT_SECRET` | JWT secret for token verification | Supabase Dashboard → Settings → API → JWT Secret |
| `RESEND_API_KEY` | Resend email API key | [resend.com/api-keys](https://resend.com/api-keys) |
| `ADMIN_NOTIFICATION_EMAIL` | Email for admin alerts | Any valid email |
